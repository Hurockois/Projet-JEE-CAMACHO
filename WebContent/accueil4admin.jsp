...
<body>
	...
	<div>
		<a href="adminUsers.jsp"> Vers administration des utilisateurs</a>
	</div>
	<div>
		...
		<tbody>
			<c:forEach var="c" items="${model.getListeC()}" varStatus="status">
				...
				<tr>
					...
					<%-- 						<c:url value="/afficherParticipant.jsp" var="url_affichage"> --%>
					<%-- 							<c:param name="code" value="${p.code}"></c:param> --%>
					<%-- 							<c:param name="op" value="1"></c:param>						 --%>
					<%-- 						</c:url> --%>
					<%-- 						<td><a href="${url_affichage}">Afficher détails</a><td>						 --%>
					<!-- 							</table> -->
					</div>
				</tr>
			</c:forEach>
